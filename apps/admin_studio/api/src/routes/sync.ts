...
syncRoute.post('/push', async (_c) => {
  ...
  (async () => {
    try {
      ...
      execSync(`pnpm wrangler d1 migrations apply valocoach-archive --remote`, {
        cwd: API_DIR,
        stdio: 'pipe',
      });
      ...
      send({
        type: 'step',
        message: 'リモートCloudflare D1にインポート中（数分かかる場合があります）...',
      });
      execSync(`pnpm wrangler d1 execute valocoach-archive --remote --file=${TMP_FILE}`, {
        cwd: API_DIR,
        stdio: 'pipe',
      });
...
syncRoute.post('/pull', async (_c) => {
  ...
  (async () => {
    try {
      ...
      execSync(`pnpm wrangler d1 execute valocoach-archive --local --file=${TMP_PULL_FILE}`, {
        cwd: API_DIR,
        stdio: 'pipe',
      });
