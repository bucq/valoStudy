...
const MINIMAP_POSITIONS = Array.from({ length: 10 }, (_, i) => 0.25 + (i / 9) * 0.7);
...
async function extractFrame(
  ...
  try {
    ...
    if (buf.byteLength > 5_000) {
      console.log(
        `[frameSamplerLocal] ${frameType} ${Math.round(position * 100)}% (${timestamp}s): ${Math.round(buf.byteLength / 1024)}KB`,
      );
    ...
    console.warn(
      `[frameSamplerLocal] ffmpeg failed at ${timestamp}s (${frameType}):`,
