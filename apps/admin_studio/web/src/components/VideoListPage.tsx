...
import { apiFetch, type Video, type VideoListResponse, type StatsResponse } from '../lib/api.ts';
import { MAPS, AGENTS, RANKS } from '../lib/valorant.ts';
...
export default function VideoListPage() {
  ...
  useEffect(() => { fetchStats(); fetchChannels(); }, [fetchStats, fetchChannels]);
  useEffect(() => { fetchVideos(applied, page); }, [fetchVideos, applied, page]);
  ...
  const handleToggleHide = async (v: EditableVideo) => {
    ...
    try {
      ...
      setVideos(vs => vs.map(x => x.id === v.id ? { ...x, isValorantCoaching: hidden ? 1 : 0 } : x));
    } catch (e) { setMsg(`エラー: ${String(e)}`); }
  };
    ...
    <div className="animate-in space-y-5">
      ...
      {msg && (
        <div className="panel p-3" style={{ borderColor: '#45D48344' }}>
          <span className="text-ok" style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem' }}>{msg}</span>
        </div>
      )}
      ...
      {stats && (
        <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(6, 1fr)' }}>
          {[
            ...
            <div key={s.label} className="stat-card" style={{ '--stat-accent': s.accent } as CSSProperties}>
              <div className="stat-num" style={{ color: s.alert && s.val > 0 ? '#FF4655' : '#C8C8D0' }}>
      ...
      <div className="panel p-3">
        ...
        <div className="flex flex-wrap gap-2 items-end">
          ...
          />
          <select className="field" style={{ width: '6.5rem' }} value={filters.map} onChange={e => setFilters(f => ({ ...f, map: e.target.value }))}>
            <option value="">全 Map</option>
            {MAPS.map(m => <option key={m} value={m}>{m}</option>)}
          </select>
          <select className="field" style={{ width: '6.5rem' }} value={filters.agent} onChange={e => setFilters(f => ({ ...f, agent: e.target.value }))}>
            <option value="">全 Agent</option>
            {AGENTS.map(a => <option key={a} value={a}>{a}</option>)}
          </select>
          <select className="field" style={{ width: '6.5rem' }} value={filters.rank} onChange={e => setFilters(f => ({ ...f, rank: e.target.value }))}>
          ...
          <select className="field" value={filters.review} onChange={e => setFilters(f => ({ ...f, review: e.target.value }))}>
            <option value="">全 Review</option>
            <option value="1">要レビュー</option>
            <option value="0">OK</option>
          </select>
          <select className="field" style={{ maxWidth: '12rem' }} value={filters.channel} onChange={e => setFilters(f => ({ ...f, channel: e.target.value }))}>
            ...
            {loading ? (
              ...
              <tr key={v.id} style={{ opacity: v.isValorantCoaching === 0 ? 0.45 : 1 }}>
                <td style={{ width: '9rem', padding: '0.5rem' }}>
                  <img src={v.thumbnailUrl} alt="" style={{ width: '8rem', height: '4.5rem', objectFit: 'cover', display: 'block' }} loading="lazy" />
                </td>
                <td style={{ maxWidth: '18rem' }}>
                  <a
                    ...
                    className="text-info"
                    style={{ fontSize: '0.82rem', fontWeight: 600, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', transition: 'color 0.12s', textDecoration: 'none' }}
                ...
                <td className="text-dim" style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.68rem', whiteSpace: 'nowrap' }}>
                  {v.channelTitle}
                </td>
                <td>
                  <select className="field" style={{ width: '6rem', fontSize: '0.72rem' }} value={v._editing.map} onChange={e => setField(v.id, 'map', e.target.value)}>
                    <option value="">—</option>
                    {MAPS.map(m => <option key={m} value={m}>{m}</option>)}
                  </select>
                </td>
                <td>
                  <select className="field" style={{ width: '6rem', fontSize: '0.72rem' }} value={v._editing.agent} onChange={e => setField(v.id, 'agent', e.target.value)}>
                    <option value="">—</option>
                    {AGENTS.map(a => <option key={a} value={a}>{a}</option>)}
                  </select>
                </td>
                ...
                <td>
                  {v.reviewNeeded
                    ...
                    : <span className="text-faint" style={{ fontSize: '0.7rem' }}>—</span>}
                </td>
                <td>
                  <div className="flex flex-col gap-1">
                    <button onClick={() => handleSave(v)} className="btn btn-info" style={{ fontSize: '0.68rem', padding: '0.25em 0.6em', whiteSpace: 'nowrap' }}>
                      保存
                    </button>
                    <button onClick={() => handleToggleHide(v)} className="btn btn-danger" style={{ fontSize: '0.68rem', padding: '0.25em 0.6em', whiteSpace: 'nowrap', opacity: v.isValorantCoaching === 0 ? 1 : 0.7 }}>
