...
import { videos, tagCorrectionRequests } from '../db/schema';
import { MAP_LABELS, AGENT_LABELS, RANK_LABELS } from '@valocoach/valorant';
...
videosRoute.get('/filters', async (c) => {
  ...
  const [maps, agents, ranks, coaches] = await Promise.all([
    db.selectDistinct({ value: videos.map })
      .from(videos)
      .where(and(isCoaching, isNotNull(videos.map)))
      .orderBy(videos.map),
    db.selectDistinct({ value: videos.agent })
      .from(videos)
      .where(and(isCoaching, isNotNull(videos.agent)))
      .orderBy(videos.agent),
    db.selectDistinct({ value: videos.rank })
      .from(videos)
      .where(and(isCoaching, isNotNull(videos.rank)))
      .orderBy(videos.rank),
    db.selectDistinct({ value: videos.channelTitle })
      .from(videos)
      .where(isCoaching)
  ...
  return c.json({
    ...
    ranks:   ranks.map(r => r.value).filter(Boolean).sort((a, b) => RANK_LABELS.indexOf(a!) - RANK_LABELS.indexOf(b!)),
    coaches: coaches.map(r => r.value).filter(Boolean),
  });
});

const correctionBodySchema = z.object({
  suggestedMap:   z.enum(MAP_LABELS   as [string, ...string[]]).optional(),
  suggestedAgent: z.enum(AGENT_LABELS as [string, ...string[]]).optional(),
  suggestedRank:  z.enum(RANK_LABELS  as [string, ...string[]]).optional(),
  note: z.string().max(200).optional(),
}).refine(
  data => data.suggestedMap !== undefined || data.suggestedAgent !== undefined || data.suggestedRank !== undefined,
...
videosRoute.post('/:id/correction', zValidator('json', correctionBodySchema), async (c) => {
  ...
  await db.batch([
    ...
    db.update(videos)
      .set({ reviewNeeded: 1, updatedAt: now })
